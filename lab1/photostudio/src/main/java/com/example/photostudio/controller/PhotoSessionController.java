...
public class PhotoSessionController {
    ...
    public ResponseEntity<List<PhotoSessionAdminResponseDto>> getPhotoSessionFormAdmin(
            @RequestParam(value = "client", required = false) String clientName, String clientLastName) {
        List<PhotoSessionAdminResponseDto> photoSessions;

        if ((clientName != null && !clientName.trim().isEmpty())
                || (clientLastName != null && !clientLastName.trim().isEmpty())) {
            photoSessions = service.getPhotoSessionForAdminClientFullName(clientLastName, clientName);
        } else {
    ...
    @GetMapping("/admin/{id}")
    public ResponseEntity<PhotoSessionAdminResponseDto> getPhotoSessionFormAdminById(@PathVariable Long id) {
    ...
    @PostMapping("/admin")
    public ResponseEntity<PhotoSessionResponseDto> createPhotoSession(@RequestBody PhotoSessionRequestDto requestDto) {
    ...
    public ResponseEntity<PhotoSessionAdminResponseDto> updatePhotoSession(@PathVariable Long id,
        ...
        PhotoSessionAdminResponseDto photoSession = service.updatePhotoSessionFormAdmin(id, adminDto);
